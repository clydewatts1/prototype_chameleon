...
* [ ] **Visual Distinction for Auto-Tools**:
  * [ ] Update `models.py`: Add `is_auto_created` boolean flag to `ToolRegistry`.
  * [ ] Update `runtime.py`: Prepend `[AUTO-BUILD]` to descriptions of auto-created tools in `list_tools`.
  * [ ] Update `add_sql_creator_tool.py`: Set `is_auto_created=True` for new tools.
  * [ ] Update `load_specs.py`: Set `is_auto_created=False` for system tools.
...
* [ ] **Separate Metadata from Data**:
  * [ ] Configure two database engines:
  ...
  * [ ] Refactor `runtime.py` to route System Tools to Metadata DB and Data Tools to Business DB.
